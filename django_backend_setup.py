#!/usr/bin/env python3
"""
Django Backend Setup for Existing Next.js Jewellery CRM
This script adds Django backend to your existing Next.js project
"""
...
def create_django_backend():
    """Create Django backend in the jewellery-crm directory"""
    
    # Navigate to the jewellery-crm directory
    ...
    if not run_command("django-admin startproject django_backend", "Creating Django backend"):
        return False
    
    # Move Django project to backend directory
    if os.path.exists("backend"):
        shutil.rmtree("backend")
    shutil.move("django_backend", "backend")
...
def setup_prisma_for_django():
    """Set up Prisma for Django backend"""
    
    # Create Prisma directory in backend
    os.makedirs("backend/prisma", exist_ok=True)
    
    # Create Prisma schema for Django
...
def main():
    ...
    if not create_django_backend():
        print("❌ Failed to create Django backend")
        return
    
    # Step 2: Setup Prisma for Django
    if not setup_prisma_for_django():
        print("❌ Failed to setup Prisma for Django")
        return
    ...
    # Step 6: Create Next.js integration
    ...
    print("\n🚀 Next Steps:")
    print("1. Install Django dependencies: cd backend && pip install -r requirements.txt")
    print("2. Set up your database and update .env.local")
    print("3. Run Prisma migrations: npm run prisma:migrate")
    print("4. Run Django migrations: npm run migrate")
    print("5. Create superuser: npm run createsuperuser")
    print("6. Start both servers: npm run dev")
    print("7. Access Next.js frontend: http://localhost:3000")
    print("8. Access Django admin: http://localhost:8000/admin")
    print("9. Access Django API: http://localhost:8000/api/v1/")
